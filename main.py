...
class APIsPlugin(Star):
    ...
    async def call_api(
        ...
        :return: (text, path, source)
                 ...
                 source = "local" 表示来自本地兜底
        """
        logger.info(f"[call_api] ===== 开始调用API =====")
        logger.info(f"[call_api] API名称: {api_data.get('name')}, URLs: {api_data.get('urls')}, params: {params or api_data.get('params')}")
        try:
            # === 外部接口调用 ===
            logger.info(f"[call_api] 准备调用 web.get_data()")
            ...
            logger.info(f"[call_api] web.get_data() 返回成功, text: {bool(api_text)}, byte: {bool(api_byte)}")
